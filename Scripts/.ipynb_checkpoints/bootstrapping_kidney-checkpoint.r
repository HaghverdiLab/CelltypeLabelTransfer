...
get_predicton <- function(sets, combinations, method, set, cutoff=0.25){
    ...
    prediction_filtered <- apply(prediction, 1, summarize_binary_prediction)
      combs <- Reduce('+', subsets)              
    prediction<- get_confidence_df(subsets, nr_sets)                
    prediction_cf <- get_confidence_df(subsets_filtered, nr_sets)
    df <- data.frame(id=rownames(combs), method=method, set=set,
                      prediction_confidence=prediction, prediction_confidenceFiltered = prediction_cf,
                      prediction_binary = prediction_binary, prediction_filtered= prediction_filtered)  
    }else{
       df <- data.frame(id=rownames(combs), prediction_confidence=rep(NA, nrow(combs)), method=method, set=set,
                    prediction_binary = prediction_binary,
                        prediction_filtered= rep(NA, nrow(combs)))   
    }
...
#itclust_pred <- do.call( plyr::rbind.fill,lapply(sequence, function(seq) get_predicton(sets_itclust,
...
seurat_pred <- do.call(plyr::rbind.fill,lapply(sequence, function(seq) get_predicton(sets_seurat,
                                                                       unlist(combinations[seq]),
                                                                       "Seurat", seq, cutoff)))
...
scn_pred <- do.call(plyr::rbind.fill,lapply(sequence, function(seq) get_predicton(sets_scn,
                                                                       unlist(combinations[seq]),
                                                                       "SCN", seq)))
...
singler_pred <- do.call(plyr::rbind.fill,lapply(sequence, function(seq) get_predicton(sets_singler,
                                                                       unlist(combinations[seq]),
                                                                       "SingleR", seq)))
...
print("Get Reference...")
ref <- read.csv("/fast/AG_Haghverdi/Carla_Moelbert/Celltype_annotation/Data/Fulldata/Kidney_Query/meta.csv")
data <- read.csv("/fast/AG_Haghverdi/Carla_Moelbert/Celltype_annotation/Results/Files/results_Kidney.csv")
print(head(data))                                  
celltypes = c("PT","Distal_Tubules", "CD_IC", "Loop of Henle","T_Cells", "Endo", "NK_Cells",  
                "B_Cells", "Macrophage")
...
print("Get files....")
predictions <- do.call(plyr::rbind.fill, list(seurat_pred, scn_pred, singler_pred)) #, itclust_pred
...
write.table(data,
            "/fast/AG_Haghverdi/Carla_Moelbert/Celltype_annotation/Results/Files/Kidney_bootstrap_mosaic.csv",
...
write.table(summary,
            "/fast/AG_Haghverdi/Carla_Moelbert/Celltype_annotation/Results/Files/Kidney_bootstrap_summary_mosaic.csv",
            col.names=T, row.names=F, quote=F,sep=",")